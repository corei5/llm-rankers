...
class PointwiseLlmRanker(LlmRanker):
    ...
    def rerank(self, query: str, ranking: List[SearchResult]) -> List[SearchResult]:
        ...
        if self.method == "qlm":
        ...
        elif self.method == "zero_shot_cot":
            prompt = (
                "Query: {query}\n"
                "Document: {document}\n"
                "Let's reason step-by-step before answering whether this document answers the query.\n"
                "Step 1: Understand the query.\n"
                "Step 2: Analyze the document's content.\n"
                "Step 3: Reason based on the document's relevance to the query.\n"
            ...
            with torch.no_grad():
                for batch_inputs in tqdm(loader):
                    ...
                    batch_inputs = batch_inputs.to(self.llm.device)
                    logits = self.llm(input_ids=batch_inputs['input_ids'],
                                      attention_mask=batch_inputs['attention_mask']).logits

                    # Assume that after reasoning, we expect a "Yes" or "No" answer to the relevance.
                    yes_id = self.tokenizer.encode("Yes", add_special_tokens=False)[0]
                    no_id = self.tokenizer.encode("No", add_special_tokens=False)[0]

                    # Evaluate the logits and apply self-reflection through reasoning
                    batch_scores = logits[:, 0, [yes_id, no_id]]
                    batch_scores = torch.nn.functional.softmax(batch_scores, dim=1)
                    scores = batch_scores[:, 0]  # assuming "Yes" indicates relevance
                    for score in scores:
                        ranking[current_id].score = score.item()
                        current_id += 1
